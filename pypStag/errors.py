...
class NoFileError(PypStagError):
    """Raised when pypStag.stagData.import find no file during the importation"""
...
class PypStagDimensionError(PypStagError):
    """Raised when unexpected dimension"""
    def __init__(self,dim,dtype=False):
        if not dtype:
            super().__init__('Error in the mesh dimension. You asked for %s dimensions'%str(dim))
        else:
            super().__init__('Error in the mesh dimension. The mesh dimension have to be an integer. You passed: '+str(type(dim)))
...
class InputGridGeometryError(PypStagError):
    ...
    def __init__(self,geom):
        super().__init__('Error on the input geometry!\n'+\
             "The proposed/detected geometry '"+geom+"' is not supported in\n"+\
             'by a pypSatg.stagData.StagData object or is incompatible with StagYY.')
...
class ExpectedReorganized2DError(PypStagError):
    """Raised when a function expects a reorganized 2D stagData object (reorganized=True)"""
    def __init__(self,txt=''):
        super().__init__('The current function expects here a 2D reorganized object.\n'+txt)

class UnExpectedReorganized2DError(PypStagError):
    """Raised when a function does not expect a reorganized 2D stagData object (reorganized=False)"""
    def __init__(self,txt=''):
        super().__init__('The current function does not expect here a 2D reorganized object.\n'+txt)
